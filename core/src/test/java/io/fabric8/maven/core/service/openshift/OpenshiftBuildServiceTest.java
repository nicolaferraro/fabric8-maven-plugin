...
public class OpenshiftBuildServiceTest {
    ...
    protected OpenShiftMockServer createMockServer(BuildService.BuildServiceConfig config, boolean success, long buildDelay, boolean buildConfigExists, boolean imageStreamExists) {
        ...
        if (!buildConfigExists) {
            mockServer.expect().get().withPath("/oapi/v1/namespaces/test/buildconfigs/" + projectName + config.getS2iBuildNameSuffix()).andReturn(404, "").once();
            mockServer.expect().post().withPath("/oapi/v1/namespaces/test/buildconfigs").andReturn(201, bc).once();
        } else {
            mockServer.expect().patch().withPath("/oapi/v1/namespaces/test/buildconfigs/" + projectName + config.getS2iBuildNameSuffix()).andReturn(200, bc).once();
        }
        mockServer.expect().get().withPath("/oapi/v1/namespaces/test/buildconfigs/" + projectName + config.getS2iBuildNameSuffix()).andReturn(200, bc).always();
        ...
        mockServer.expect().post().withPath("/oapi/v1/namespaces/test/buildconfigs/" + projectName + config.getS2iBuildNameSuffix() + "/instantiatebinary?commit=").andReturn(201, imageStream).once();

        mockServer.expect().get().withPath("/oapi/v1/namespaces/test/builds").andReturn(200, builds).always();
